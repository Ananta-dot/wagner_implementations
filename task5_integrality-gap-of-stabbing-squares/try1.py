...
def build_base_array(n, size, duplicates=False):
    duplicates = False
    if not duplicates:
        arr = [1 + i*size for i in range(n)]
    else:
        arr = []
        pair_count = (n+1)//2 
...
def calc_score(state):
    ...
    baseX = build_base_array(n, size, duplicates=True)
    baseY = build_base_array(n, size, duplicates=True)
    ...
    for i, b in enumerate(x_bits):
        ...
        idx2 = (i+1) % n
        if b==1:
            arrX[idx1], arrX[idx2] = arrX[idx2], arrX[idx1]
