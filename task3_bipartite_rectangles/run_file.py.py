...
def build_base_array(n_):
    """
    For n_=2 => length=8 => [1,1,2,2,3,3,4,4]
    We'll do an array of length=4*n_ => 2n_ distinct, each repeated => total 4n_.
    """
    arr= []
    for val in range(1,2*n_+1):
        arr.extend([val,val])
...
def calc_score_and_arrays(state):
    ...
    for i in range(1, 2 * n + 1):
        # Find all indices where arrH equals i
    ...
    for i in range(2*n):
        for j in range(i+1, 2*n):
            same_c = ( (i<n and j<n) or (i>=n and j>=n) )
            does_int = intersect_strict(rects[i], rects[j])
            if same_c:
                if not does_int:
                    score+=1
            else:
...
def generate_session(agent, n_sess, verbose=1):
    ...
    final_arrays_h= np.zeros((n_sess, 4*n), dtype=int)
    final_arrays_v= np.zeros((n_sess, 4*n), dtype=int)
...
super_final_h  = np.empty((0, 4*n), dtype=int)
super_final_v  = np.empty((0, 4*n), dtype=int)
